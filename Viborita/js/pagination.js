...
(async () => {
  ...
  let textPagination = () =>
    (document.getElementsByClassName(
      "paginationText"
    )[0].textContent = `Registros del ${
      firstPaginatedUser() + 1
    } al ${lastPaginatedUser()} de un total de ${users.length}`);
  ...
  let removeUser = (pIconDelete) => {
    for (let i = 0; i < pIconDelete.length; i++) {
      pIconDelete[i].addEventListener("click", function (e) {
        ...
        let idItem = parseInt(
          itemToDelete.getElementsByTagName("td")[0].textContent
        );
        let usernameItem =
          itemToDelete.getElementsByTagName("td")[1].textContent;
  ...
  let editUser = (pIconEdit) => {
    for (let i = 0; i < pIconEdit.length; i++) {
      pIconEdit[i].addEventListener("click", function (e) {
        ...
        let idItem = parseInt(
          itemToEdit.getElementsByTagName("td")[0].textContent
        );
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            let usernameEdited =
              itemToEdit.getElementsByTagName("td")[1].textContent;
            let fullnameEdited =
              itemToEdit.getElementsByTagName("td")[2].textContent;
            let emailEdited =
              itemToEdit.getElementsByTagName("td")[3].textContent;
            ...
            Swal.fire(
              `El usuario ${usernameEdited.toUpperCase()} se ha editado correctamente`,
              "",
              "success"
            );
